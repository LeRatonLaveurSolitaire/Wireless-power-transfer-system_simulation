...
def extract_noise(
    ...
    Args:
        clean_signal (list): Signal without noise. Defaults to None.
        noisy_signal (list): Signal with noise (PRBS is injected). Defaults to None.
        sampling_period (float) : Sampling period of the noisy and clean signals. Defaults to None.
        f0 (float) : Resonance frequency used for phase syncronistion.  Defaults to None.
...
def main():
    ...
    noise, noise_spectrum = extract_noise(
    ...
    # plt.plot([i for i in range(len(current))], current_prbs)
    ...
    impulse = X_corr_function(noise,prbs)
    print(len(noise),len(prbs),len(impulse))
    ...
    phase_gain = -np.mean(
        np.angle(
            smoothed_impedance[
                np.where((freqs_noise < 110_000) & (freqs_noise > 75_000))
            ]
        )
    )
