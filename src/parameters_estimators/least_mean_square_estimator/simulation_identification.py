...
from utils.plot_function import bode_plot, loss_plot
...
def main() -> None:
    ...
    param_vect = [0.1, 1e-12, 1e-6, 42 * 1e-9, 0.3]  # R_l, M²,  L2, C2, R2
    ...
    cost_s = cost_function_s(
        impedance_at_freq, tested_frequency, param_vect, estimated_system_s
    )
    ...
    while i < 90:  # cost_s > 1:
        ...
        cost_s = mse_cost_function(
            impedance_at_freq, tested_frequency, param_vect, estimated_system_s
        )
        costs.append(cost_s)

        # print(
        #     f"Iter {i}, cost : {cost_s:.4G}, param_vect : R_l = {param_vect[0]:.2G}, M = {np.real(param_vect[1]**0.5) *1e-6:.2G} µH, L2 = {param_vect[2] *1e-6 :.2G} µH"  # , R_l : {param_vect[0]:6.3f}, M : {param_vect[1]**0.5 * 1e6 :6.0f} µH, L2 : {param_vect[2] * 1e6 :.2f} µH, , C2 = {param_vect[3] * 1e9:.2G} nF"
        # )

    print(
        f"Estimated parameters : R_l : {param_vect[0]:.2G}, M : {np.real(param_vect[1]**0.5) *1e-6:.2G} µH, L2 : {param_vect[2] *1e-6:.2G} µH, C2 = {param_vect[3]:.2G} nF"
    )

    print(f"Estimated parameters : R_l : 2, M : 7 µH, L2 : 84 µH, C2 = 42 nF")
    estimated_system_s.reciever = reciever_s
    # estimated_system_s.M = real_system.M
    estimated_system_s.print_info()
    print(
        f"cost for estimated system : {mse_cost_function(impedance_at_freq, tested_frequency, param_vect, estimated_system_s):.3f}"
    )
    print(
    ...
    loss_plot(
        costs,
        mse_cost_function(impedance_at_freq, tested_frequency, param_vect, real_system),
    )

    systems = [real_system]  # , estimated_system_s]
